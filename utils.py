...
class Metrics:
    ...
    def cpc(self):
        y_pred = self.y_pred.squeeze().cpu().detach().numpy()
        y_true = self.y_true.squeeze().cpu().detach().numpy()
        return 2 * sum(map(min, y_pred, y_true)) / (sum(y_pred) + sum(y_true))

    def mape(self, epsilon=1e-0):
        return np.mean(np.abs(self.y_pred - self.y_true) / (self.y_true + epsilon))
...
def load_checkpoint(folder, model, optimizer=None):
    ...
    """
    if not os.path.exists(folder):
        raise FileNotFoundError(f"File does not exist: {folder}!")
