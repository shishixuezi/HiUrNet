...
from torch_geometric.nn import MessagePassing, HGTConv
from torch_geometric.typing import EdgeType, EDGE_TYPE_STR_SPLIT
...
def set_hetero_masks(
    ...
    for module in model.modules():
        if isinstance(module, torch.nn.ModuleDict):
            for edge_type in mask_dict.keys():
                if edge_type in module:
                    ...
                    edge_level_module = module[EDGE_TYPE_STR_SPLIT.join(edge_type)]
                else:
                    continue
        ...
        elif isinstance(module, HGTConv):
            ...
            edge_keys_dict = [EDGE_TYPE_STR_SPLIT.join(k) for k in edge_index_dict.keys()]
