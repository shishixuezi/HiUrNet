...
def population():
    ...
    return pd.read_csv(path, header=0, names=['KEY_CODE', 'population'])
...
def poi():
    ...
    dataset.drop(dataset.columns[[2, 6]], axis=1, inplace=True)

    dataset['KEY_CODE'] = dataset['KEY_CODE'].astype('long')
    return dataset


def road_density(compact=False):
